...
namespace device
{
  ...
  static constexpr std::uint32_t pll_sys_postdiv2 = 2;
    ...
    || defined(MICRO_OS_PLUS_DEVICES_RASPBERRY_PI_CORE_RP2350_RISCV)
  ...
  // `runtime_init_clocks.c`), combined with `xosc_init()`/`pll_init()`
  // (hardware_xosc/hardware_pll). Only clk_ref, clk_sys and clk_peri
  // are configured; clk_usb/clk_adc/clk_hstx/clk_rtc and the tick
  // generators are not used by this platform and are left at their
  // reset state.
...
// The CMSIS core headers (pulled in via RP2040.h/RP2350.h in
// micro-os-plus/device.h) define __CM_CMSIS_VERSION; when present, the
// CMSIS peripheral structs (CLOCKS, XOSC, ...) are available.
  ...
  // A second, functionally identical implementation is needed because
  // CMSIS is Arm-specific, so the CMSIS peripheral structs (CLOCKS,
  // XOSC, ...) are not available on the RP2350 RISC-V cores; here the
  // registers are accessed directly via the RP2350_REG32 macro.
  // The Arm cores could use this non-standard macro too, but the
  // CMSIS version above is preferred there; if the SDK ever provides
  // equivalent peripheral structs for RISC-V, this branch can go.
  std::uint32_t
  system_clock::initialise (void)
  {
    // Disable resus, in case it was left enabled by previous software.
  ...
  system_clock::clock_get_frequency_hz (void)
  {
    ...
    std::uint32_t fbdiv_int = RP2350_REG32 (PLL_SYS_BASE, PLL_FBDIV_INT_OFFSET)
                              & PLL_FBDIV_INT_BITS;
    std::uint32_t postdiv1 = (RP2350_REG32 (PLL_SYS_BASE, PLL_PRIM_OFFSET)
                              & PLL_PRIM_POSTDIV1_BITS)
                             >> PLL_PRIM_POSTDIV1_LSB;
    std::uint32_t postdiv2 = (RP2350_REG32 (PLL_SYS_BASE, PLL_PRIM_OFFSET)
...
#endif // defined(__CM_CMSIS_VERSION)
